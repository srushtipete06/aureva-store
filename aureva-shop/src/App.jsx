...
function App() {
  ...
  const [view, setView] = useState('shop'); // 'shop' ya 'checkout' ya 'success'
  ...
  // 💳 ASLI DYNAMIC PAYMENT AND ORDER PLACING LOGIC
  const handlePlaceOrder = async (e) => {
    e.preventDefault();

    try {
      // 1. Backend se Razorpay Order ID mangwana
      ...
      if (!paymentData.success) {
        alert("Payment initiate nahi ho paya bhai! Check backend console.");
        return;
      }

      // 2. Razorpay Popup Options Setup
      const options = {
        key: "rzp_test_T2iL4JJgye7uDR", // 👈 YAHAN APNI KEY_ID CHIPKAO (rzp_test_...)
        amount: paymentData.order.amount,
        currency: "INR",
        name: "AUREVA Luxury",
        description: "Fine High Jewelry Purchase",
        order_id: paymentData.order.id, // Real order id from backend
        handler: async function (response) {
          // 3. Payment successful hone ke baad order MongoDB mein save karna
          ...
          if (orderData.success) {
            ...
            alert("Payment ho gayi par database mein order save nahi hua!");
          }
        },
        prefill: {
        ...
        theme: { color: "#b3925c" } // Royal Gold Theme
      };
      ...
      alert("Kuch gadbad ho gayi payment ke waqt!");
    }
  };
    ...
    <div className="min-h-screen bg-[#faf9f6] text-[#2c2a29] font-serif relative">
          ...
          <main className="max-w-7xl mx-auto px-8 py-16">
            {products.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-xl italic text-gray-500">Database empty hai bhai! MongoDB Atlas mein data daalo.</p>
              </div>
            ) : (
                ...
                {products.map((product) => (
                  <div key={product._id} className="bg-white group border border-[#e5e1da] overflow-hidden transition-all duration-300 hover:shadow-xl">
                    ...
                    <div className="p-6 text-center">
                      ...
                      <p className="text-[#b3925c] text-lg font-semibold tracking-wider mb-6">
                        ₹{product.price.toLocaleString('en-IN')}
                      </p>
                      <button 
                        onClick={() => addToCart(product)}
                        className="w-full border border-[#2c2a29] text-[#2c2a29] py-3 text-xs uppercase tracking-widest font-semibold bg-transparent transition-all duration-300 hover:bg-[#2c2a29] hover:text-white"
                      >
                        Add To Cart
                      </button>
                    </div>
      ...
      {view === 'checkout' && (
        <main className="max-w-4xl mx-auto px-8 py-16">
          <button onClick={() => setView('shop')} className="text-sm uppercase tracking-widest text-gray-400 hover:text-[#b3925c] mb-8 block">
            ← Back to Gallery
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="bg-white border border-[#e5e1da] p-8">
              ...
              <form onSubmit={handlePlaceOrder} className="space-y-4">
                ...
                <button type="submit" className="w-full bg-[#2c2a29] text-white py-4 text-xs uppercase tracking-widest font-semibold hover:bg-[#b3925c] transition-all duration-300 mt-6">
                  Place Order (Pay Now)
                </button>
              </form>
            </div>
      ...
      {view === 'success' && (
        <main className="max-w-md mx-auto text-center py-28 px-8">
          ...
          <button onClick={() => setView('shop')} className="bg-[#2c2a29] text-white px-8 py-3 text-xs uppercase tracking-widest font-semibold hover:bg-[#b3925c] transition-all duration-300">
            Continue Shopping
          </button>
        </main>
      )}
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-[#e5e1da] pt-6 mt-6">
                ...
                <button 
                  ...
                  className="w-full bg-[#2c2a29] text-white py-4 text-xs uppercase tracking-widest font-semibold hover:bg-[#b3925c] transition-all duration-300"
                >
                  Proceed to Checkout
                </button>
              </div>
