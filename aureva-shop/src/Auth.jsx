...
function Auth({ setView, onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        setMessage(data.message + ' Please proceed to log in.');
        setIsLogin(true);
  ...
  return (
    <main className="max-w-md mx-auto px-8 py-16 bg-white border border-[#e5e1da] mt-12 shadow-md font-serif">
      <button onClick={() => setView('shop')} className="text-xs uppercase tracking-widest text-gray-400 hover:text-[#b3925c] mb-6 block">
        ← Back to Shop
      </button>
